...
export function IconButton({ icon, onPress, label, color = colors.ink, className = '', size = 22, disabled = false, shape = 'regular' }: { icon: IconName; onPress: () => void; label: string; color?: string; className?: string; size?: number; disabled?: boolean; shape?: 'regular' | 'small' | 'stepper' }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} disabled={disabled} accessibilityState={{ disabled }} onPress={onPress} hitSlop={4} className={`${dimensions} items-center justify-center rounded-full active:opacity-60 ${disabled ? 'opacity-40' : ''} ${className}`}><Icon name={icon} size={size} color={color}/></Pressable>;
}
...
export function Tile({ title, subtitle, icon, tone = 'neutral', onPress, onLongPress, brightness, chevron = false, active, height = 80, grow = true }: { title: string; subtitle?: string; icon: IconName; tone?: Tone; onPress: () => void; onLongPress?: () => void; brightness?: number; chevron?: boolean; active?: boolean; height?: number; grow?: boolean }) {
  const t = tones[tone];
  return <Pressable onPress={onPress} onLongPress={onLongPress} accessibilityRole={active === undefined ? 'button' : 'switch'} accessibilityState={active === undefined ? undefined : { checked: active }} accessibilityLabel={`${title}${subtitle ? ', ' + subtitle : ''}`} accessibilityHint={onLongPress ? 'Hold to adjust brightness' : undefined} className={`relative min-w-0 ${grow ? 'flex-1' : ''} flex-row items-center gap-3 overflow-hidden rounded-[24px] px-4 active:opacity-75 ${t.tile}`} style={{ minHeight: height, borderCurve: 'continuous' }}>
    ...
    <View className="min-w-0 flex-1 py-2"><Label className={`text-[13px] font-medium leading-[17px] ${t.text}`}>{title}</Label>{subtitle && <Label className={`mt-0.5 text-[11px] leading-[14px] ${t.text}`}>{subtitle}</Label>}</View>
