...
export function EmergencyScreen({ onNavigate, header }: ScreenProps) {
  ...
  return <View className="flex-1 bg-card" onLayout={event => setWidth(event.nativeEvent.layout.width)}><PageScroll maxWidth={1200}>
    ...
    <ScrollView horizontal scrollEnabled={compact} showsHorizontalScrollIndicator={false} snapToInterval={compact ? cameraWidth + 12 : undefined} decelerationRate="fast" onScroll={event => setPage(Math.round(event.nativeEvent.contentOffset.x / (cameraWidth + 12)))} scrollEventThrottle={80} contentContainerStyle={{ gap: 12 }}><CameraCard label={t('emergency.livingRoom')} height={compact ? 176 : 300} width={cameraWidth}/><CameraCard garden height={compact ? 176 : 300} width={cameraWidth}/></ScrollView>
