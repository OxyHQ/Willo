...
export function EmergencyScreen({ onNavigate, header }: ScreenProps) {
  ...
  return <View className="flex-1 bg-card" onLayout={event => setWidth(event.nativeEvent.layout.width)}><PageScroll maxWidth={1200}>
    {header}
    <Label className="pb-5 pt-1 text-[21px] leading-[29px]">Emergency · Smoke detected</Label><View className="mb-4 flex-row items-center gap-3 rounded-full bg-home-surface px-3 py-3"><Icon name="alert" size={20} color={themeColors.error}/><Label className="flex-1 text-[12px]">Check what’s happening. Use caution.</Label></View>
