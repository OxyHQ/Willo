import { Icon, type IconName, Label, Tile, colors } from '@willo/ui';
...
export function HomeScreen({ onNavigate }: ScreenProps) {
  ...
  const visible = selected === 'Favorites' ? order.map(id => base[id])
    : selected === 'All' ? [...order.map(id => base[id]), ...Object.values(base).filter(card => !order.some(id => id === card.id))].map(card => ({ ...card, lane: undefined }))
