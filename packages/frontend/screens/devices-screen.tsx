...
export function DevicesScreen({ onNavigate }: ScreenProps) {
  ...
  return <View className="flex-1 bg-white"><ClassicHeader title="Devices" onNavigate={onNavigate}/><ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} className="flex-1" contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 82 }}>
    <SectionTitle>Front room</SectionTitle><View className="flex-row gap-2">{device('tv', 'TV', 'tv')}<Tile title="Thermostat" subtitle="Indoor 70°" icon="thermometer" tone="peach" chevron onPress={() => onNavigate('home')}/></View>
    <SectionTitle>Living room</SectionTitle><View className="gap-2"><View className="flex-row gap-2">{device('living-lamp', 'Lamp', 'light', true)}<Tile title="Camera" icon="camera" tone="blue" chevron onPress={() => setSheet({ kind: 'camera', title: 'Living room camera' })}/></View><View className="flex-row gap-2">{device('blinds', 'Blinds', 'blinds')}{device('vacuum', 'Vacuum', 'vacuum')}</View></View>
    <SectionTitle>Office</SectionTitle><View className="gap-2"><View className="flex-row gap-2">{device('plug', 'Smart plug', 'plug')}{device('office-lamp', 'Lamp', 'light', true)}</View><View className="flex-row gap-2">{device('office-blinds', 'Blinds', 'blinds')}<View className="flex-1"/></View></View>
  </ScrollView><AddButton onPress={() => setSheet({ kind: 'menu', title: 'Add to your home', options: [
