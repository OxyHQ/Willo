...
/** Same reasoning as `LightTile` above, for the demo's own local `living-lamp`/`office-lamp`. */
function DemoLightTile({ id, title }: { id: DeviceKey; title: string }) {
...
export function DevicesScreen({ onNavigate, header }: ScreenProps) {
  const { state, dispatch, setSheet, devices } = useHome();
