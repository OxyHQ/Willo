...
export function AssistantScreen({ onNavigate }: ScreenProps) {
  ...
  return <KeyboardAvoidingView className="flex-1 bg-white" behavior={process.env.EXPO_OS === 'ios' ? 'padding' : undefined}>
    <View className="flex-row items-center gap-2 px-2 pb-1"><IconButton icon="back" label="Back to home" onPress={() => onNavigate('home')}/><Label className="text-[13px] text-home-muted">Ask Spring Street</Label></View>
    <ScrollView contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} className="flex-1" contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 6, paddingBottom: 20 }}>
    ...
    <View className="bg-white px-4 pb-3 pt-2"><View className="flex-row items-center rounded-full bg-home-surface pl-4 pr-1"><TextInput accessibilityLabel="Ask Spring Street" placeholder="Ask Spring Street" placeholderTextColor={colors.muted} value={input} onChangeText={setInput} onSubmitEditing={submit} returnKeyType="send" maxLength={300} className="h-[48px] flex-1 text-[13px] text-home-ink"/><IconButton icon="send" size={19} label="Send question" disabled={!input.trim()} color={colors.onSky} onPress={submit}/></View><Pressable onPress={() => setSheet({ kind: 'message', title: 'About this preview', description: 'The disclaimer and sample conversation reproduce the supplied reference. This demo does not use Gemini or any other AI service.' })} accessibilityRole="button" className="py-3"><Label className="text-center text-[9px] text-home-muted">Gemini can make mistakes, so double check it. <Label className="text-[9px] text-home-muted underline">Learn more</Label></Label></Pressable></View>
