...
export function SettingsScreen({ onNavigate, header }: ScreenProps) {
  ...
  return <View className="flex-1 bg-card"><PageScroll>{header}<PageColumns><View>
    ...
    {hasHomeToShow ? <>
      ...
      <CardStrip>{mini(t('settings.miniHallwayThermostat'), 'thermometer', () => onNavigate('home'))}{mini(t('settings.miniFrontDoorLock'), 'lock', () => onNavigate('home'))}{mini(t('settings.officeWifi'), 'wifi', () => show(t('settings.officeWifi'), t('settings.officeWifiDescription')))}{mini(t('settings.livingRoom'), 'light', () => onNavigate('devices'))}</CardStrip>
    </> : (
      <View className="mt-3 items-center gap-3 rounded-[24px] bg-muted p-6">
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={setupStage === 'needs-pairing' ? t('settings.connectHomeAssistant') : t('settings.setUpHome')} onPress={() => onNavigate('onboarding')} className="rounded-full bg-primary-subtle px-6 py-3"><Label className="text-[13px] font-medium text-primary-text">{setupStage === 'needs-pairing' ? t('settings.connectHomeAssistant') : t('settings.setUpHome')}</Label></Pressable>
