...
export function SettingsScreen({ onNavigate }: ScreenProps) {
  ...
  return <View className="flex-1 bg-white"><ClassicHeader title="Settings" onNavigate={onNavigate}/><PageScroll><PageColumns><View>
    ...
    <CardStrip>{mini('Willo\nSecurity', 'shield', () => show('Willo Security', 'This is a recreation of the reference service tile, not an active integration.'))}{mini('Works with\nOxy', 'link', () => show('Works with Oxy', 'This reference service tile does not link an Oxy account.'))}{mini('Video', 'video', () => show('Video', 'Camera images are local stills cropped from the supplied references.'))}{mini('Music', 'speaker', () => show('Music', 'No music account connected.'))}</CardStrip>
