...
export function SettingsScreen({ onNavigate, header }: ScreenProps) {
  const { setSheet, setupStage, homeName, demoMode, setDemoMode } = useHome();
  ...
  return <View className="flex-1 bg-card"><PageScroll>{header}<PageColumns><View>
    ...
    {hasHomeToShow ? <>
      <Pressable onPress={() => show('Home details', `${homeName}\n\nNo address is stored for this Home yet.`)} accessibilityRole="button" className="mb-2 mt-3 flex-row items-center justify-between"><Label className="text-[34px] font-semibold">{homeName}</Label><View className="h-7 w-7 items-center justify-center rounded-full bg-home-surface"><Icon name="chevron" size={14}/></View></Pressable>
      ...
      <View className="mt-3 items-center gap-3 rounded-[24px] bg-home-surface p-6">
    ...
    <View className="mt-3 flex-row items-center justify-between rounded-[22px] bg-home-surface p-4"><View className="flex-row items-center gap-3"><Icon name="bell" size={20}/><Label className="text-[13px]">Notifications</Label></View><Switch accessibilityLabel="Enable demo notifications" value={notifications} onValueChange={setNotifications} trackColor={{ false: '#d5d9de', true: themeColors.primarySubtle }} thumbColor={notifications ? themeColors.primary : '#fff'}/></View>
    <View className="mt-2 flex-row items-center justify-between rounded-[22px] bg-home-surface p-4"><View className="min-w-0 flex-1 flex-row items-center gap-3"><Icon name="sparkle" size={20}/><View className="min-w-0 flex-1"><Label className="text-[13px]">Demo mode</Label><Label className="text-[11px] text-muted-foreground">Preview a fully-stocked smart home instead of your real devices</Label></View></View><Switch accessibilityLabel="Enable demo mode" value={demoMode} onValueChange={setDemoMode} trackColor={{ false: '#d5d9de', true: themeColors.primarySubtle }} thumbColor={demoMode ? themeColors.primary : '#fff'}/></View>
