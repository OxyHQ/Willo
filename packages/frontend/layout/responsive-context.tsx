...
export function ResponsiveProvider({ children, preview = false }: { children: React.ReactNode; preview?: boolean }) {
  ...
  const width = size?.width ?? (preview ? 360 : window.width);
  const height = size?.height ?? (preview ? 780 : window.height);
