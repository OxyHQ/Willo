...
import { BREAKPOINTS, getLayoutMetrics, packMasonry } from '../layout/metrics.ts';
import { isNavigationActive, modernTabs, classicTabs } from '../components/navigation-items.ts';
import { homeReducer, initialHomeState } from '../state/home-reducer.ts';
for (const [width, rail, sidebar] of [[320, false, false], [599, false, false], [600, true, false], [1024, true, false], [1439, true, false], [1440, true, true]]) {
...
test('four-column semantic lanes follow the supplied tablet reference', () => {
    ...
    assert.ok(result.placements[3].x > result.placements[2].x);
});
test('semantic lanes are ignored at two and three columns', () => {
    ...
    assert.ok(result.placements[1].x > 0);
});
...
test('new reference tiles use the existing reducer rather than separate desktop state', () => {
    ...
    for (const id of ['fan', 'garage', 'speaker', 'floor-lamp']) {
