 ...
 *
 * Done on the JSON rather than with `LottieView`'s `colorFilters` because the
 * web renderer ignores `colorFilters`. Returns `animation` itself when no
 * colour matched.
 */
export function recolorLottie(animation: AnimationObject, palette: LottiePalette): AnimationObject {
  if (palette.size === 0) {
    return animation;
  }
